...
def _ensure_data_dir() -> None:
    db_path = Path(get_settings().db_file)
...
def _stats() -> dict:
    ...
    if not db_path.exists():
        return {"players": 0, "total_points": 0, "best_plate": None}
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*), COALESCE(SUM(total_points), 0), MAX(best_plate) FROM players")
    row = cur.fetchone() or (0, 0, None)
    conn.close()
    return {"players": row[0], "total_points": row[1], "best_plate": row[2]}
...
async def lifespan(app: FastAPI):
    ...
    os.environ.setdefault("OWNER_ID", str(settings.owner_id))
    _ensure_data_dir()
    task: asyncio.Task | None = None
    application = None
    if settings.telegram_polling_enabled and settings.telegram_bot_token:
        from autobot import build_application, configure_commands
        ...
        log.info("telegram polling started for autobot")
        task = asyncio.create_task(_poll_guard())
    try:
        yield
    finally:
        if task:
...
app = FastAPI(title="Autobot", version="1.0.0", lifespan=lifespan)
...
@app.get("/")
def root():
    return {"ok": True, "service": "autobot", "miniapp": "/miniapp"}
...
def admin_summary(x_admin_token: str | None = Header(default=None)):
    ...
    settings = get_settings()
    return {"service": "autobot", "stats": _stats(), "mini_app_url": settings.telegram_mini_app_url}
...
async def max_webhook(request: Request):
    ...
    return {"ok": True, "platform": "max", "accepted": bool(payload), "stage": "autobot-max-bridge"}
