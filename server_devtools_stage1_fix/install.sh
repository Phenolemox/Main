...
log "SERVER DEVTOOLS STAGE 1 FIX"
echo "host=$(hostname) user=$USER_NAME time=$(date -u +%Y-%m-%dT%H:%M:%SZ)"

log "CREATE ROOT PATHS SAFELY"
mkdir -p "$BIN"
...
test -w /opt/data/ai-control-room/server || { echo "AI_CONTROL_ROOM_SERVER_NOT_WRITABLE"; exit 1; }
...
for c in prettier biome tsc vite html-validate playwright; do printf '%-14s ' "$c"; command -v "$c" || echo missing; done
...
Installed utilities for diagnostics and product development:

- jq, ripgrep, fd, tree, sqlite3, shellcheck, yamllint, htop, ncdu.
- Python QA venv in /opt/devtools/python-qa: ruff, mypy, pytest, bandit, pip-audit, pre-commit, pipdeptree, httpie, rich.
- Node/MiniApp tools: prettier, Biome, TypeScript, Vite, html-validate, Playwright package.
- Helpers: server-toolbox, qa-poker, miniapp-check, telegram-bot-check, telegram-set-commands.

Rules:
- Do not print Telegram tokens.
- Project-local dependencies remain inside each repo.
- Heavy Playwright browsers are not installed yet.
MD

server-toolbox | sed -n '1,180p'
telegram-bot-check || true

echo "SERVER_DEVTOOLS_STAGE1_FIX_DONE"
